leResTenFold<-function(hidden,inicializacao,epocas,shortcut,nDadosIn,nDadosOut,erros)
{
   setwd("c:\\TreinaRedes\\TenFold\\RESFILES\\")
   tabela<-NULL
   ...
   for (h in hidden)
   {
      for (ini in inicializacao)
      {
         for (epc in epocas)
         {
             ...
             {
                dados<-juntaRes(h,ini,epc,short,nDadosIn,nDadosOut,"tenfold",1:10)
